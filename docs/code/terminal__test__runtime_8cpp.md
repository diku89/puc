...
Run `bazel run //puc-cli/test_apps:terminal-test` in a real terminal for the complete plan. Append `-- --list` to inspect stable test names without entering terminal mode, or `-- --test clipboard-paste` to execute only the named check.
...
[Source](../../puc-cli/test_apps/terminal/terminal_test_runtime.cpp)
